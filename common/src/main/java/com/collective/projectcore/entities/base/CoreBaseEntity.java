...
public abstract class CoreBaseEntity extends TameableEntity {
    ...
    public CoreBaseEntity(EntityType<? extends TameableEntity> entityType, World world) {
    ...
    public boolean canBreedWith(AnimalEntity other) {
        if (other instanceof CoreBaseEntity coreBaseEntity) {
            if (this.getClass().equals(coreBaseEntity.getClass())) {
                if (this.getGender() != coreBaseEntity.getGender()) {
                    if (!this.isMother() && !this.isFather() && !coreBaseEntity.isMother() && !coreBaseEntity.isFather()) {
                        if (this.getBreedingTicks() <= 0 && coreBaseEntity.getBreedingTicks() <= 0) {
                            if (this.getMateUUID().equals(coreBaseEntity.getUuidAsString()) && coreBaseEntity.getMateUUID().equals(this.getUuidAsString())) {
                                return !this.isPregnant() && !coreBaseEntity.isPregnant();
                            } else if (!this.isMated() && !coreBaseEntity.isMated()) {
                                return !this.isPregnant() && !coreBaseEntity.isPregnant();
                            }
                        }
                    }
    ...
    public void breed(ServerWorld world, AnimalEntity other) {
        if (other instanceof CoreBaseEntity mate) {
    ...
    // --- Data Registry ------------------------------------------------------------------------------------------
    static {
        AGE_SCALE = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.FLOAT);
        AGE_TICKS = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.INTEGER);
        BREEDING_TICKS = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.INTEGER);
        FATHER_TICKS = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.INTEGER);
        GENDER = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.INTEGER);
        HOME_POS = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.BLOCK_POS);
        HUNGER = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.INTEGER);
        HUNGER_TICKS = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.INTEGER);
        MATE_UUID = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.STRING);
        MATE_VARIANT = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.STRING);
        MOTHER_TICKS = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.INTEGER);
        MOTHER_UUID = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.STRING);
        PREGNANCY_TICKS = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.INTEGER);
        VARIANT = DataTracker.registerData(CoreBaseEntity.class, TrackedDataHandlerRegistry.STRING);
